...
function [param,pred]=kalman_armaxVM(y,u,Aord,Bord,Cord,Re,Rw,V0,m0,predlength)
...
% State space equations w. states being ARMAX param: x(t)=[a1(t) ... ap(t) c1(t) ... cq(t) b0(t) ... bs(t) mean(t)]'

% x(t) = A*x(t-1)+e = I*x(t-1)+e(t)
% y(t) = C(t)*x(t) = [-y(t-1)...-y(t-p) ehat(t-1|t-2)...ehat(t-q|t-q-1) u(t)...u(t-s) 1]*x(t)+w(t)
...
A =eye(Totparam+1); % Define system matrices
C=zeros(1,Totparam+1); 
...
for k=1:predlength
    ...
    % Below we create C(t+k|t-1) ie the C to calculate yhat(t+k|t-1) next
    % round
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
